...
const SevaServeAgentPanel = () => {
  ...
  const [editingExistingMedia, setEditingExistingMedia] = useState<any[]>([]); // URLs already uploaded
  const [editingNewFiles, setEditingNewFiles] = useState<File[]>([]); // New files added during edit
  const [editingNewPreviews, setEditingNewPreviews] = useState<any[]>([]);
  ...
  // Start Edit Mode with Text & Media
  const startEdit = (msg: any) => {
    setEditingMessageId(msg.id);
    setEditingText(msg.text || "");
    setEditingExistingMedia(msg.serverMedia || []); // Raw server URLs
    setEditingNewFiles([]);
    setEditingNewPreviews([]);
  };

  // Add New Files in Edit Mode
  const handleEditFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;

    const newFiles = Array.from(files);
    setEditingNewFiles((prev) => [...prev, ...newFiles]);
  ...
  const removeExistingMedia = (index: number) => {
    setEditingExistingMedia((prev) => prev.filter((_, i) => i !== index));
  };
  ...
  // -------------------------------------------------------------
  // COMPLETE EDIT & RE-EXECUTE PIPELINE
  // -------------------------------------------------------------
  const saveEdit = async (msgId: any) => {
    if (
      !editingText.trim() &&
      editingExistingMedia.length === 0 &&
    ...
    try {
      ...
      // Step A: Upload any NEW files attached during edit mode
      if (editingNewFiles.length > 0) {
        ...
        if (editingText.trim()) {
          uploadFormData.append("message", editingText);
        }
        ...
        if (uploadRes.success) {
          ...
          if (Array.isArray(uploadData.media)) {
            newlyUploadedUrls = uploadData.media.map((m: any) =>
              typeof m === "string" ? m : m.url || m.path || ""
            );
          } else if (uploadData.url) {
            newlyUploadedUrls = [uploadData.url];
          }
        }
      }

      // Combine existing valid server URLs + newly uploaded URLs
      const allFinalMediaUrls = [
      ...
      // Step B: Re-run Analysis Pipeline if media exists
      if (allFinalMediaUrls.length > 0) {
        const analyzeFormData = new FormData();
        allFinalMediaUrls.forEach((url: string) => {
          analyzeFormData.append("media[]", url); // Now guaranteed to be valid HTTP URLs
        });
        if (editingText.trim()) {
          analyzeFormData.append("message", editingText);
        }
        ...
        if (analyzeRes.success) {
          ...
          const catId = firstIssue.category_id || selectedCategory?.id || 1;
          const subCatId = firstIssue.subcategory_id || "";
          const serviceId = firstIssue.service_id || "";
          const issueTitle = firstIssue.issue || editingText || "Issue Request";
          const problemDesc =
            firstIssue.problem_description || editingText || "";
          const aiAnalysisMsg = firstIssue.ai_analysis || "Analysis complete.";
        ...
        // If all images deleted and only text remains
    ...
    try {
      if (filesToSend.length > 0) {
        ...
        if (uploadRes.success) {
          ...
          // Add message to UI with real server URLs
          const userMsg = {
            id: Date.now(),
            sender: "user",
            text: text,
            media: localPreviews,
            serverMedia: validMediaUrls,
            type: "text",
          };
          setMessages((prev) => [...prev, userMsg]);
          ...
          <div className="messages-inbox-grp">
            {messages.map((msg) => (
              <React.Fragment key={msg.id}>
                {/* USER MESSAGE WRAPPER WITH FULL INLINE EDIT BOX */}
                {msg.sender === "user" && (
                    ...
                    >
                      <div className="chat">
                        {editingMessageId === msg.id ? (
                          ...
                          >
                            {/* Existing Media Items in Edit Box */}
                            {editingExistingMedia.length > 0 && (
                              <div className="d-flex gap-2 flex-wrap">
                                {editingExistingMedia.map((url, idx) => (
                                  <div
                                    key={idx}
                            ...
                            {/* New Files Added in Edit Box */}
                            {editingNewPreviews.length > 0 && (
                              <div className="d-flex gap-2 flex-wrap">
                                {editingNewPreviews.map((file, idx) => (
                                  <div
                                    key={idx}
                                    style={{ position: "relative" }}
                                  >
